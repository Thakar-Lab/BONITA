...
from experiments import findRPKNnoise
...
# set up style for a talk rather than paper.... replace set style and context in setupPlot with a call to talkStyle
def talkStyle():
	sns.set_style("dark")
...
def plot_box(df, xval, yval, title, xlabeler, ylabeler, plotname, limits):
	...
	sns.set_style("ticks")
	fig, ax = plt.subplots(figsize=[2.63,2.4])
	ax=sns.boxplot(ax=ax, data=df,x=xval, y=yval, color='black', fliersize =1., linewidth = .85)
	plt.rc('font', size=10)          # controls default text sizes
	plt.rc('axes', titlesize=10)     # fontsize of the axes title
	plt.rc('axes', labelsize=10)    # fontsize of the x and y labels
	plt.rc('xtick', labelsize=10)    # fontsize of the tick labels
...
def compareIndividualsNodeWise(truthList, testList, model1s, model2s,covs, equivs):
	...
	for node in range(0,len(modeler.nodeList)):
		...
		for k in range(len(truthList)):
			...
			# find structural distance across equivalents
			minSD=1000
			for equiver in equivs[k][node]:
				equivSet=Set([])
				for i in range(0,len(equiver)):
					if equiver[i]==1:
						for nodeToAdd in model2s[k].andNodeList[node][i]:
							equivSet.add(nodeToAdd)
				tempSD=len(truthSet.difference(equivSet))+len(equivSet.difference(truthSet))
				if minSD>tempSD:
					minSD=tempSD
			equivSDs[k]=equivSDs[k]+minSD
	...
	return equivSDs, sampleEquivRT, equivNodeRTsens, nodePsens,nodepPPV,sampleSens, samplePPV, nodeSens, nodePPV, sampleRT, nodeRTs, edgeSens, edgePPV, SDs, nodeSDs, len(modeler.nodeList), inDegrees, inCoV
...
# analyze experiment varying noise- in samples, omics levels etc. 
...
def analyzeEdgeNoiseExperiment(codes, end):
	...
	# df=pd.DataFrame(sensspeclist)
	df=setupPlot(sensspeclist)
	sns.axes_style('whitegrid')
	print(df)
	# plot_box(df, 'Gen 2', 'SD', 'SD by noise', 'Multiple of Edges Added', 'Structural Distance', 'SD_box1.png',True)
	# plot_box(df, 'Gen 2', 'SD', 'SD by noise', 'Multiple of Edges Added', 'Structural Distance', 'SD_box2.png',False)
	# plot_box(df, 'Gen 2', 'Proportion Rules True', 'SD by noise', 'Multiple of Edges Added', 'Rules Strue', 'RT_box2.png',False)

	plot_box(df, 'Gen 2', 'Equivalent SD', 'Equivalent SD by noise', 'Multiple of Edges Added', 'Structural Distance', 'SD_box8_plos.png',True)
	plot_box(df, 'Gen 2', 'Equivalent SD', 'Equivalent SD by noise', 'Multiple of Edges Added', 'Structural Distance', 'SD_box9_plos.png',False)
...
if __name__ == '__main__':
	...
	for file in os.listdir("gpickles"):
		if file.endswith(".gpickle"):
			codes.append(file[:-8])
	print(codes)
	analyzeEdgeNoiseExperiment(codes,'')